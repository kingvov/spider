...
@Component
@Slf4j
public class NationalAreaFillProcessor implements PageProcessor {
    ...
    public void process(Page page) {
        try {
            ...
            if(url.equals(NATIONNAL_AREA_URL)){
                List<ErrorUrl> errorUrls = errorUrlMapper.selectList(new QueryWrapper<ErrorUrl>().eq("flag", 0));
                List<String> collect = errorUrls.stream().map(ErrorUrl::getUrl).collect(Collectors.toList());
                for(ErrorUrl errorUrl : errorUrls){
                    errorUrl.setFlag(1);
                    errorUrlMapper.update(errorUrl, new UpdateWrapper<ErrorUrl>().eq("url", errorUrl.getUrl()));
                }
